    ...
    try {
      ...
      const sizeGuard = new Transform({
        transform(chunk, encoding, callback) {
          ...
          if (downloadedBytes > maxUploadBytes) {
      ...
      // بعض لوحات Xtream ترسل Content-Length غير دقيق أو لحجم مختلف.
      // لذلك لا نعتمد على الهيدر لتحديد الحجم؛ نحسب البايتات الفعلية أثناء التنزيل.
      await pipeline(
      ...
      if (stat.size > maxUploadBytes) {
        throw new Error("حجم الملف أكبر من حد Telegram Local Bot API المسموح.");
      }

      // Supplying a real file path lets form-data calculate Content-Length
      // reliably and avoids "form-data: terminated" with live upstream streams.
