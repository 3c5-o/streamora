...
function createTelegramService() {
  ...
  async function downloadXtreamToFile({ upstreamUrl, tempPath, maxUploadBytes }) {
    ...
    // Probe the first small range. If the Xtream server supports byte ranges,
    // download in chunks so a dropped connection only retries a small part.
    for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
      try {
        ...
        if (rangeSupported) {
          ...
          // Server ignored Range and started a normal full response.
          await fs.promises.writeFile(tempPath, Buffer.alloc(0));
          offset = 0;
          const received = await appendResponse(response, "a");
          offset += received;
    ...
    while (totalSize === null || offset < totalSize) {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
        try {
          ...
          if (response.status === 416 && totalSize !== null && offset >= totalSize) {
            completed = true;
            break;
          }
          ...
          if (match) {
            ...
            if (match[3] !== "*") {
              ...
              if (discoveredTotal > maxUploadBytes) {
                throw new Error("حجم الملف الفعلي أكبر من حد Telegram Local Bot API المسموح.");
              }
          ...
          // Restore offset from what is safely present on disk and retry that chunk.
  ...
  async function uploadMediaStream({ target, source, mediaType, mediaId, extension, title, caption }) {
  ...
  async function deliverAuthorizedMedia({ chatId, source, mediaType, mediaId, extension, title, seriesId, season }) {
    ...
    // Telegram often rejects raw Xtream HTTP URLs because panels return a
    // generic/HTML content type. Route delivery through our HTTPS signed proxy.
    ...
    if (botApiBaseUrl) {
      // Local Telegram Bot API: stream the authorized Xtream media as multipart
      // instead of asking Telegram's cloud servers to fetch a remote URL.
      sent = await uploadMediaStream({
        target,
        source,
        mediaType,
        mediaId,
      ...
      try {
        ...
        throw new Error(
          "Telegram Cloud Bot API رفض جلب هذا الملف كرابط. " +
          "الفيديوهات الكبيرة أو MKV تحتاج Local Bot API ورفع multipart."
        );
      }
    }
